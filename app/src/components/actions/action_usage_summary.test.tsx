...
function conversation(actionId: string, cardPath: string, totalTokens: number): AgentConversation {
    return {
        actionId,
        cardPath,
        completedAt: 'now',
...
describe('ActionUsageSummary', () => {
    it('shows filtered tokens and hides lines when history has no commits', () => {
        render(<ActionUsageSummary
            ...
            conversations={[
                conversation('implement', 'design/F-1.md', 12),
                conversation('review', 'design/F-1.md', 30),
                conversation('implement', 'design/F-2.md', 40),
            ]}
            history={[]}
        />)
