...
describe('UsageMetricsService', () => {
    ...
    it('creates one header and appends escaped token rows with empty account fields', async () => {
        ...
        await service.recordTokenUsage(destination, 'codex', TOKEN_USAGE, Date.parse('2026-08-17T10:11:12.000Z'));
        await service.recordTokenUsage(destination, 'claude', TOKEN_USAGE, Date.parse('2026-08-17T10:12:13.000Z'));
    ...
    it('writes and restores quoted multiline limit identities', async () => {
        ...
        await service.recordAccountUsage(destination, 'codex', codexSnapshot(10, 1_800_000_000, limitId));
        const restartedService = new UsageMetricsService();
        await restartedService.recordAccountUsage(destination, 'codex', codexSnapshot(14, 1_800_000_000, limitId));
    ...
    it('serializes concurrent writes to one project file', async () => {
        ...
        await Promise.all(Array.from({ length: 5 }, (_, index) => (
            service.recordTokenUsage(destination, 'codex', TOKEN_USAGE, Date.parse('2026-08-17T10:00:00.000Z') + index)
        )));
    ...
    it('maps provider windows and calculates first, same-window, reset, and negative deltas', async () => {
        ...
        await service.recordAccountUsage(destination, 'codex', codexSnapshot(20));
        await service.recordAccountUsage(destination, 'codex', codexSnapshot(25));
        await service.recordAccountUsage(destination, 'codex', codexSnapshot(23));
        await service.recordAccountUsage(destination, 'codex', codexSnapshot(4, 1_800_003_600));
        await service.recordAccountUsage(destination, 'claude', claudeSnapshot(10, 30));
    ...
    it('restores last valid baselines after restart and skips unchanged snapshots', async () => {
        ...
        await firstService.recordAccountUsage(destination, 'codex', codexSnapshot(20));
        await writeFile(filePath, `${await readFile(filePath, 'utf8')}malformed,row\r\n`, 'utf8');
        const restartedService = new UsageMetricsService();

        expect(await restartedService.recordAccountUsage(destination, 'codex', codexSnapshot(20))).toBe(false);
        expect(await restartedService.recordAccountUsage(destination, 'codex', codexSnapshot(27))).toBe(true);
    ...
    it('separates a new row from valid existing CSV without a trailing newline', async () => {
        ...
        await restartedService.recordAccountUsage(destination, 'codex', codexSnapshot(25));
    ...
    it('rejects malformed destinations, token usage, and account snapshots without creating a file', async () => {
        const { destination, filePath } = await createDestination();
        const service = new UsageMetricsService();

        expect(await service.recordTokenUsage(destination, 'codex', { ...TOKEN_USAGE, inputTokens: -1 }, Date.now())).toBe(false);
        expect(await service.recordTokenUsage(destination, 'codex', { ...TOKEN_USAGE, totalTokens: 12 }, Date.now())).toBe(false);
        expect(await service.recordAccountUsage(destination, 'claude', { ...claudeSnapshot(10, 20), available: false })).toBe(false);
        expect(await service.recordAccountUsage(destination, 'codex', codexSnapshot(Number.NaN))).toBe(false);
        expect(await service.recordTokenUsage({ ...destination, projectFolder: '..' }, 'codex', TOKEN_USAGE, Date.now())).toBe(false);
    ...
    it('reports read and append failures without rejecting and allows later retry', async () => {
        ...
        expect(await service.recordTokenUsage(destination, 'codex', TOKEN_USAGE, Date.now())).toBe(false);
        expect(await service.recordTokenUsage(destination, 'codex', TOKEN_USAGE, Date.now())).toBe(true);
    ...
    it('writes one account snapshot independently to concurrent project destinations', async () => {
        ...
        await Promise.all([
            service.recordAccountUsage(first.destination, 'claude', snapshot),
            service.recordAccountUsage(second.destination, 'claude', snapshot),
        ]);
