...
describe('ActionRunnerService', () => {
    ...
    it('returns terminal failure for runtime selection error', async () => {
        ...
        await expect(runToCompletion(runner, {
            actionId: 'main', context, runInput: { model: 'retired-model' },
        })).resolves.toMatchObject({ failure: expect.stringContaining('Unknown model'), status: 'failed' })
    ...
    it('composes real execution and command collaborators with ordered events', async () => {
        ...
        expect(events.filter(({ status, type }) => status === 'completed' && type === 'action').map(({ actionId, phase }) => ({
            actionId, phase,
        }))).toEqual([
