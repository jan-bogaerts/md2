...
class RemoteControlService {
    ...
    onActionConversationViewed(client, id) {
        if (!this.dispatcher) throw new Error('Remote-control dispatch is not configured');

        const subscriptionId = crypto.randomUUID();
        const cleanup = this.dispatcher.invoke('onActionConversationViewed', [
            (event) => sendJson(client, { event: 'conversationViewed', payload: { event, requestId: id, subscriptionId } }),
        ]);
        this.addSubscription(client, subscriptionId, cleanup);
