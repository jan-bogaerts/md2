...
Allow Electron to register linked Git worktrees, assign cards to them, prepare a dedicated worktree for actions that set `needsWorkTree`, and show worktree/agent state on each card.
...
- Separately, Electron can create a branch and linked worktree while preparing an action whose definition sets `needsWorkTree`.
...
- When `needsWorkTree` is true for an unassigned card, derive a proposed branch name from the card id and title, create and register the linked worktree, and assign it to the card.
- When `needsWorkTree` is true without card context, ask the user for a branch name before creating and registering the execution worktree.
- Electron performs Git preparation before starting the action process. A branch collision, invalid name, or other Git error is shown to the user. The preparation UI allows a custom branch name and retry.
...
- A branch-name collision or invalid proposed name keeps the action unstarted and allows a custom branch-name retry.
...
- Add Git integration tests using temporary repositories and real `git worktree add`: common-directory validation, registration, existing assignment, action-created worktree, proposed/custom branch names, collisions, detached worktrees and stale paths.
...
- Add Electron action-runner tests for primary/card execution, worktree creation, linked actions, schedules and Git-error retry.
...
- Actions with `needsWorkTree` use an assigned worktree or prepare one through the specified proposed/custom branch-name flow.
