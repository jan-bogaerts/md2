...
- `desktop/schedule_store.js` — schedule file read/write/validation and status transitions, written as pure functions over the JSON model (no timers, no service state).
...
- `ActionSchedulerService` keeps project lifecycle, run execution via the agent runner, and event subscription, composing the two modules; its public API and the bridge contract stay unchanged.
...
- Public service API and bridge contract unchanged; desktop test suite and lint pass after every commit.
