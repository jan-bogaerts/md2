...
Schedule actions by stable action `id`: at a specific time, when the agent's next time slot starts, or after another action completes. The scheduler runs in Electron and delegates execution to the same Electron action runner as manual and state-triggered runs.

## Current state

The existing scheduler persists action names and contains a second Electron chain runner separate from the React `ActionRunner`. This must migrate to action ids and the single Electron runner from [[F-010c]].

## implementation details

- Define a typed schedule model with schedule id, action id, context, trigger, created timestamp, and schedule status.
- Supported triggers are `at` with a timestamp, `agentSlot`, and `afterAction` with an action id.
...
- Add `Schedule` before `Run` in the action popup. It opens the trigger picker and sends `{ actionId, context, trigger }` to Electron.
...
- Resolve `agentSlot` with the configured desktop command whose output is a timestamp and fail clearly when unavailable.
- Fire `afterAction` when the referenced action id completes; renaming either action does not affect the schedule.
...
- Time, agent-slot, and after-action triggers fire through the single Electron action runner.
...
- Missing action ids, invalid schedule files, and unavailable agent-slot configuration report visible errors without dropping other schedules.
- Tests cover persistence, id-based restore, timer reconciliation, all trigger types, runner delegation, terminal states, and both cancellation paths.
