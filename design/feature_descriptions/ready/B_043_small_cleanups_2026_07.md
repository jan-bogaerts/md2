...
## Items
- **Dead branch in the action runner**: `ActionRunner.runMain` (`app/src/services/action_runner.ts`) ends with `if (action.type !== 'cmd') return ''` after both `'agent'` and `'cmd'` have already returned — unreachable; delete (the trailing `return ''` alone suffices, or make the type switch exhaustive).
...
- No unreachable code remains in `ActionRunner.runMain`.
