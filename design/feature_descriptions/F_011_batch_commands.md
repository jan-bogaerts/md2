...
policy:
  checkLinting: true
  requireTests: true
internalId: e60d33fa-52ba-4b6f-936f-85affc3e58d4
---
...
- Extend the F-010 action model with generated `command` actions sourced from configured script folders. Every generated action has a stable `id` that does not depend on its editable name.
...
- Execute scripts through the Electron action runner with explicit interpreters: Windows command processor for `.bat`/`.cmd`, Windows PowerShell 5.x for `.ps1` and a configured bash executable for `.sh`.
...
- Treat generated script actions like normal F-010 actions for `onBefore`, `on`, `onAfter`, `onState`, `needsWorkTree`, circular-call checks, cancellation, logs and UI filtering. Every linked action is referenced by `id`.
...
- Script actions participate in F-010 ID-based chaining, output matching, state triggers, worktree preparation, cancellation and circular-call validation.
