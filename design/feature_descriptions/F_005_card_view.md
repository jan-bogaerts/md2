...
## Current state
Data loading from [[F-002]] already creates `ProjectCard`s from markdown parsing helpers; `DataService` can create and save cards. These helpers should move behind the shared parsing service before extending parsing behavior. `ProjectWorkspace` still shows active cards as a flat `CardSelectButton` list with a plain textarea editor. `CardHeader` has `id`, `title`, `status`, `owner` and `affects`, but not parsed `after` ordering or `policy`; `CardTypeConfig` has no color; no drag-and-drop library is installed.
