...
* Render mindmap connections without default arrow markers. Keep connection SVG below node components. **Behind a node** means a crossing line is visually occluded by the node because the edge layer has lower stacking order; it does not mean rerouting the curve around that node.
