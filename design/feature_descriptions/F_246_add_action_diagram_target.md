...
To make this work, we add to project config section, a new config value ´digram footer´, which is a markdown text added to every diagram prompt.

The prompt should contain instruction on where to save file, this is done with new placeholder ´diagram-file´

When action is started, we calculate filename and pass to prompt reslover.