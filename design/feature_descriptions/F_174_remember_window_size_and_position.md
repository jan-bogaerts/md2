...
- Add `electron-window-state` as a runtime dependency in `desktop/package.json` and update `desktop/package-lock.json`. It owns geometry validation, move and resize tracking, persistence, and maximized-state restoration; do not duplicate this behavior with `electron-store`.
