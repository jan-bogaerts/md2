...
- An agent change updates dependent model, access, approval, and thinking values in one store command and one persistence request. While a conversation waits for input, same command marks settings changed so closing and reopening popup cannot bypass restart behavior from B_94. Successful application clears only transient dirty marker, not saved selection.
...
- Preserve saved values when profile or capability configuration later changes. Show unavailable saved choice explicitly and block Run with clear validation message until user selects supported value; do not silently replace persisted choice.
...
- Unavailable saved configuration remains visible and cannot start run until corrected.
