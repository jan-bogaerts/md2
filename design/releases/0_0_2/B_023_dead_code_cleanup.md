...
policy:
  checkLinting: true
  requireTests: true
internalId: 72ee2dff-9375-4f9b-8079-5d457cc3a4b2
---
...
- ~~**Dead config entries**~~ **done** — `react.showStartupSplash` gates the splash in `app.tsx`, `connection.githubScopes` feeds `github_auth_service` (F-031). _(Update 2026-07-11: `connection.githubScopes` and the whole Connection config section were later removed with OAuth — see [[F_034_github_oauth_cors_proxy]]. Auth is now PAT-only, which takes no scopes.)_
