...
affects:
agents:
  - design/releases/0_6_0/card__fa429997-fee2-4e1d-a21f-3ea586ab12ae.json
policy:
changedFiles:
...
* `ActionAgentInteraction` renders conversation, then `ActionPromptOwner`. When a structured agent question is pending, it constructs `ActionAgentQuestionOwner` and passes that React element through `ActionPromptOwner` into `ActionAgentPrompt` as `questionsPanel`. This feature-specific JSX prop violates the component ownership rule and must be removed as part of this feature.
...
## Question component ownership refactor

* Remove the `questionsPanel?: ReactNode` prop from both `ActionPromptOwner` and `ActionAgentPrompt`. `ActionAgentInteraction` must not construct or pass `ActionAgentQuestionOwner` as JSX.
* `ActionAgentPrompt` owns the prompt/question layout, so it must render `ActionAgentQuestionOwner` directly in the question region. Pass only the required data, service, and callback inputs through `ActionPromptOwner`, including the run binding and any restored-question data; do not replace `questionsPanel` with another component, render-function, `children`, or generic JSX prop.
* Derive whether the question region is present from the bound run's pending question and the restored-question data. Keep the subscription in the smallest component that needs that value for layout and rendering.
* Preserve both verified `ActionPromptOwner` call-site behaviors: the owner rendered by `ActionAgentInteraction` may show live or restored agent questions, while the command prompt rendered by `CommandAction` remains question-free. Use an explicit data/configuration input only if it is required to distinguish those call sites.

* Extend sizing logic in `ActionAgentPrompt`. While a question is pending, track whether prompt or question panel is primary. Question panel is primary when it first appears.
...
* Update `action_agent_prompt.test.tsx`. Replace old question-first shrink expectation with primary-region allocation coverage, and exercise questions through the concrete question data/binding contract rather than a supplied JSX panel. Keep `action_agent_question.grouped.test.tsx` coverage for question cap and scrolling, updated so it no longer passes `questionsPanel`.
...
* No feature-specific question component, render function, `children`, or JSX value is passed through `ActionPromptOwner` or into `ActionAgentPrompt`; `ActionAgentPrompt` renders `ActionAgentQuestionOwner` in the layout it owns.
